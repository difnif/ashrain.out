...
// 아쉬레인의 수학 교습 방식이 어떤 학문적 전통 위에 서 있는지 — 학자·사상·문헌.
...
const SCHOLARS = [
  {
    ...
    ash: "아쉬레인이 외심·내심을 '거리가 일정한 점'으로, 원과 타원을 '등거리 조건의 형제'로 잇고, 기하 개념마다 \"이 조건이 어떤 도형을 낳는가\"를 묻는 이유가 여기에 있습니다. 중2 외심이 고등 원의 방정식·이차곡선으로 이어지는 예습 포인트의 뼈대예요.",
  ...
  },
  {
    ...
    ash: "아쉬레인이 +·-를 '크기'가 아니라 방향으로 가르치는 근거입니다. 양수는 굳이 +가 필요 없는 자연스러운 세기의 수, -는 시계 반대 방향 같은 '반대 방향' 표시. 음×음=양은 외울 규칙이 아니라 반대의 반대라는 당연한 사건이 됩니다.",
  ...
  },
  {
    ...
    ash: "부등식에서 상수항을 더하면 범위가 수평이동하고, 계수를 곱하면 범위가 늘어난다 — 경계값의 이동과 거리로 부등식을 읽는 아쉬레인의 방식이 정확히 변환의 관점입니다. 이 감각이 함수의 평행이동으로, 다시 벡터로 이어지도록 복습·예습 포인트를 설계합니다. '대학 수준이다 싶을 만큼 자세한 설명'의 원전이 바로 클라인이에요.",
  ...
  },
  {
    ...
    ash: "약수(約數)를 '실로 묶어 줄이는' 그림에서 시작하고, 소수(素數)를 '물들이기 전 흰 실'로 만나고, '왜 이 이름일까' 패널을 모든 개념에 붙이는 것 — 아쉬레인의 용어 분해 교습이 프로이덴탈의 노선입니다. 개념의 탄생 설화를 알면, 공식은 암기가 아니라 재발견이 됩니다.",
  ...
  },
  {
    ...
    ash: "\"수학적으로 오류만 아니라면 근본 설명을 최대한 자세히 — 빼는 건 쉽다\"는 아쉬레인의 원칙이 스켐프의 답입니다. 1이 소수가 아닌 진짜 이유(유일성 보호), 계수가 '개수'가 아닌 이유까지 내려가는 설명은 사치가 아니라, 하위권 학생에게 더 필요한 안전망입니다.",
...
const BOOKS = [
  ["폴 록하트, 『Measurement』 (Harvard, 2012)", "자·컴퍼스가 아니라 '측정과 거리'라는 물음에서 기하 전체를 다시 짓는 책. 아쉬레인의 '거리에서 출발하는 기하' 서사와 가장 가까운 현대 저작."],
    ...
    <div className={`ph-root ph-${theme}`}>
      ...
      <div className="ph-wrap">
        ...
        <p className="ph-intro">
          아쉬레인의 설명 방식은 취향이 아니라 전통 위에 서 있습니다. 용어를 글자까지 뜯어보고,
        ...
        {SCHOLARS.map((s) => (
          <div key={s.id} className="ph-card">
            ...
            <div className="ph-ash"><b>아쉬레인에서는</b><p>{s.ash}</p></div>
