...
export default function PhotoMark({ useCur = false }) {
  ...
  useEffect(() => { setK(0); setMode("show"); }, [p]);
  useEffect(() => {
    if (!p?.question) return;
    saveDevice({ id: sid, feature: "mark", question: p.question, figure_note: p.figure_note, unit: p.unit_guess, grade: p.std?.item_grade || null, result: { marks: seq.length } });
  }, [p, sid, seq.length]);
  ...
  return (
    <SolveShell title="표시 연습하기" back="#/solve/photo" toast={toast}
      ...
      <div className="sv-card">
        ...
        <SentenceMarks tokens={ex.tokens} marks={shown} />
        {last && (
        ...
        {seq.length > 0 && (
          <div className="ph-step-bar">
            <button className="sv-btn" onClick={() => setK((v) => Math.max(0, v - 1))} disabled={k === 0}>◀ 이전</button>
            <span className="cnt">{Math.min(k, seq.length)}/{seq.length}</span>
            {!done
              ? <button className="sv-btn pri" onClick={() => setK((v) => Math.min(seq.length, v + 1))}>{k === 0 ? "첫 표시 보기" : "다음 ▶"}</button>
              : <button className="sv-btn" onClick={() => setK(0)}>처음부터</button>}
          </div>
        )}
        {seq.length > 1 && !done && <div style={{ textAlign: "right", marginTop: 6 }}><button className="sv-btn sm ghost" onClick={() => setK(seq.length)}>한 번에 보기</button></div>}
      </div>

      <div className="sv-card">
        <div className="sv-sec" style={{ marginTop: 0 }}>표시 읽는 법</div>
        <MarkLegend />
        <div className="sm-wrap sm-legend" style={{ padding: 0, marginTop: 4 }}><span><span className="sm-t sm-dist">구하는 것</span> 물결 = 마지막에 확인할 것</span></div>
      </div>
