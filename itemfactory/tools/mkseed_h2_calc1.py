...
def lm_t6():
    return T(LM, 6, LM_B, title="좌극한과 우극한 — 조각 정의 함수",
        ...
        sol2=[("우극한: [[lim(x, {p}, pow(x,2) {sgn(a)}, +)]] = {p}² {sgn(a)} = {R}", "x ≥ {p} 쪽"), ("좌극한: [[lim(x, {p}, {co(b)}x {sgn(c)}, -)]] = {b} × {pn(p)} {sgn(c)} = {Lf}", "x < {p} 쪽"), ("합: {R} + {pn(Lf)} = {ans}", None, ("{ans}", "합"))],
...
def _lm7_piece():
    ...
    return T(LM, 7, LM_B, title="함수의 연속 — 두 조각 다항식이 연속이 되도록 하는 상수",
        ...
        sol2=[("좌극한: {b} × {pn(p)} {sgn(c)} = {Lf}", "x < {p} 쪽"), ("우극한 = 함숫값: {p}² + a = {p2} + a", "x ≥ {p} 쪽"), ("{p2} + a = {Lf} → a = {ans}", None, ("{ans}", "a"))],
