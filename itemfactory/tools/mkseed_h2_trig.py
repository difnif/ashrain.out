...
def _t7_rows():
    out = {}
    for (x, y, z, ang) in ((3, 5, 7, 120), (5, 3, 7, 120), (7, 8, 13, 120), (8, 7, 13, 120), (5, 16, 19, 120), (16, 5, 19, 120), (9, 15, 21, 120), (3, 8, 7, 60), (8, 3, 7, 60), (5, 8, 7, 60), (8, 5, 7, 60), (7, 15, 13, 60), (15, 7, 13, 60), (8, 15, 13, 60), (15, 8, 13, 60), (5, 21, 19, 60), (3, 4, 5, 90), (4, 3, 5, 90), (5, 12, 13, 90), (12, 5, 13, 90), (8, 15, 17, 90), (15, 8, 17, 90), (7, 24, 25, 90), (6, 8, 10, 90)):
        desc = f"삼각형 ABC에서 sin A : sin B : sin C = {x} : {y} : {z}"
        if Fraction(ang) in _nums(desc):
            continue
        cv = Fraction(x * x + y * y - z * z, 2 * x * y)
        out[f"r{x}_{y}_{z}"] = {"DESC": desc, "V": ang, "LAW": "사인법칙 a : b : c = sin A : sin B : sin C, 코사인법칙", "STEP": f"a : b : c = {x} : {y} : {z}이므로 a = {x}k, b = {y}k, c = {z}k로 놓고 cos C = (a² + b² − c²)/(2ab) = [[frac({x * x + y * y - z * z}, {2 * x * y})]] = {_fm(cv)}", "RES": f"cos C = {_fm(cv)}이고 0° < C < 180°이므로 C = {ang}°"}
    return out
...
def tr_t7():
    return T(TR, 7, TR_B, title="사인법칙의 비와 코사인법칙 — 각의 크기",
        skill="sin A : sin B : sin C = a : b : c로 변의 비를 잡고 코사인법칙으로 각을 구하기", axis={"변의 비": "3:5:7, 7:8:13, 3:8:7, 3:4:5 등 24가지", "각": "60°, 90°, 120°"}, disc="사인의 비를 변의 비로 바꾼 뒤 세 변의 코사인법칙으로 각을 결정하는가", diff=3,
        params=[{"name": "f", "values": {"in": list(T7_ROWS)}}], table={"key": "f", "rows": T7_ROWS},
        derive={"ans": "V"}, cost=["ans"], verify=["ans == V"],
        q="{DESC}일 때, 각 C의 크기를 구하시오.", answer="[[deg({ans})]]",
        sol1="사인법칙 a/sin A = b/sin B = c/sin C = 2R에서 a : b : c = sin A : sin B : sin C이다. 따라서 변의 비를 알 수 있고, 세 변의 비에 코사인법칙 cos C = (a² + b² − c²)/(2ab)를 쓰면 k가 약분되어 각이 정해진다.",
        sol2=[("{LAW}", "공식 고르기"), ("{STEP}", "대입"), ("{RES}", None, ("[[deg({ans})]]", "C"))],
        sol3=["cos C의 값이 특수각(½, 0, −½)에 해당하는지, 삼각형의 내각 범위(0° < C < 180°) 안에서 하나로 정해지는지 확인한다. 따라서 C = [[deg({ans})]]이다.", "{RES}", "C = [[deg({ans})]]"],
        model="{LAW}에서 {STEP}이므로 {RES}이다.",
        rubric=[("변의 비", 3, "{STEP} 꼴로 변의 비에 코사인법칙을 썼다.", "사인의 비를 변의 비로 바꾸지 못했으면 인정하지 않는다."), ("각", 2, "C = [[deg({ans})]]{eul(ans)} 구했다.", "cos 값에서 각을 잘못 읽으면 1점.")],
