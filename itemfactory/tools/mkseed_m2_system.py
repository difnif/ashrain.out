...
def ss_t3():
    return tpl(SS, 3, SS_BASE,
        ...
        sol2_fig=steps([
            ...
            {"text": "{ss}a = {ra}  →  a = {a}", "hint": "(③ × {pn(x0)}) − (④ × {pn(y0)}) — b 소거", "marks": [{"on": "{ss}a", "note": "{x0}² + {y0}²"}]},
        ...
        sol3="a = {a}, b = {b}{eul(b)} 넣으면 {co(a)}x {BS} = {c1}, {co(b)}x {NAS} = {c2}이고, x = {x0}, y = {y0}{eul(y0)} 대입하면 {a} × {pn(x0)} + {pn(b)} × {pn(y0)} = {c1}, {b} × {pn(x0)} − {pn(a)} × {pn(y0)} = {c2}{ro(c2)} 모두 성립한다. 따라서 {ASK} = {ans}이다.",
        sol3_fig=steps(["{a} × {pn(x0)} + {pn(b)} × {pn(y0)} = {c1} ✓", "{b} × {pn(x0)} − {pn(a)} × {pn(y0)} = {c2} ✓", "{ASK} = {ans}"]),
